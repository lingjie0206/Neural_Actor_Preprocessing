...
def convert_func(inputs):
    return convert_from_openpose(src=inputs[0], dst=inputs[1])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--novideoextract', action='store_true')
    args = parser.parse_args()
    if os.path.isdir(args.path):
        if not args.novideoextract:
            ...
            subs_full = sorted(glob(join(args.path, 'images', '*')))
            subs=[ss.split('/')[-1] for ss in subs_full]
            print(subs)
        if not args.no2d:
            ...
            for sub in subs:
                ...
                if os.path.exists(annot_root):
                    continue
                all_srcs.append((join(args.path, 'openpose', sub), annot_root))
            import multiprocessing as mpl
            with mpl.Pool(32) as pool:
                pool.map(convert_func, all_srcs)
