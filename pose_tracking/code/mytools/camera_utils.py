...
def read_camera(intri_name, extri_name, cam_names=[0,1,2,3]):
    ...
    for cam in cam_names:
        ...
        # print(R.shape, Tvec.shape, RT.shape, P[cam].shape, cams[cam]['K'].shape, cams[cam]['dist'].shape)
    cams['basenames'] = cam_names
    return cams

def read_camera_nsvf(intri_folder, extri_folder, cam_names=[0,1,2,3]):
    assert os.path.exists(intri_folder), intri_folder
    assert os.path.exists(extri_folder), extri_folder
    ...
    for cam in cam_names:
        cams[cam] = {}
        # 内参只读子码流的
        intri_fname=os.path.join(intri_folder, '0_train_{:04d}.txt'.format(int(cam)))
        cams[cam]['K']=np.loadtxt(intri_fname)
        cams[cam]['invK'] = np.linalg.inv(cams[cam]['K'])

        extri_fname=os.path.join(extri_folder, '0_train_{:04d}.txt'.format(int(cam)))
        RT44=np.loadtxt(extri_fname)
        RT44=np.linalg.inv(RT44)
